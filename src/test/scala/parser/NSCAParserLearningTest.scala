...
class NSCAParserLearningTest extends SpecificationWithJUnit {
    ...
    "NSCAParserTest" should {
      ...
      "parse one record successfully" in new Context {
        ...
        record.get.httpStatusCode.get === 200
        record.get.bytesSent.get === 12988
      ...
      //38.123.140.114 -  -  [18/Jul/2014:00:00:03 +0000] "GET /cgi-bin/wspd_cgi.sh/WService=wsbroker1/webtools/oscommand.w HTTP/1.1" 301 - "-" "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 5.1; Trident/4.0)"
      ...
      "parse one different record successfully" in new Context {
        ...
        record.get.httpStatusCode.get == 301
        record.get.bytesSent == None
